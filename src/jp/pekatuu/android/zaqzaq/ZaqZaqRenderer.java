...
public class ZaqZaqRenderer implements Renderer {
	private GLText testText = new GLText("test".toCharArray(), 0.12f * GameData.height, GameData.width / 2f,
...
//		draw
	...
	private void shot() {
		if (nextTimeShoot > currentTime) return;
		float size2 = aimStick.x * aimStick.x + aimStick.y * aimStick.y;
		if (size2 < 0.1f) return;
	...
	public void onSurfaceChanged(GL10 gl, int width, int height) {
		...
		testText = new GLText("test".toCharArray(), 0.12f * GameData.height, GameData.width / 2f,
				0.25f * GameData.height, 0f, 1, 1);
	}
